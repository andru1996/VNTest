...
public class GameController : Jun_ComponentSingletonObject<GameController> 
{
	...
	void Start () 
	{
		if(gameLevel.RandomPoint)
        {
			gameLevel.SetRandomTargetPoint();
		}
	...
	void Update () 
	{
        ...
        if (gamingData.gameTime > LoseTime && gameStatus != GameStatus.GameLose)
        {
            SetGameStatus(GameStatus.GameLose);
        }
	...
	void Gaming ()
	{
		...
		if(Input.GetMouseButtonUp (0))
		{
			...
			case GameLevel.LevelType.GetAllCrystal:
				...
				{
					SetGameStatus (GameStatus.GameWin);
				}
	...
	void AddLatticeToTargetLine (GameLevel.TargetPointInfo targetPoint,Lattice addLattice)
	{
		...
		if(gameAudioSetting.linkAudio.Length > 0)
        {
