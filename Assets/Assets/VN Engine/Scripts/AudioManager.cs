...
namespace VNEngine
{
    public class AudioManager : MonoBehaviour
    {
        ...
        public void Set_Music(AudioClip new_music, float volume = 1)
        {
            background_music_audio_source.clip = new_music;
            background_music_audio_source.loop = true;
            background_music_audio_source.volume = Mathf.Clamp(volume, 0, music_volume);
            background_music_audio_source.Play();
        }

        public void ChangePlayingMusicVolume(float volume)
        {
            background_music_audio_source.volume = Mathf.Clamp(volume, 0, music_volume);
        }

        // Starts the new ambience music and starts it playing
        public void Set_Ambience(AudioClip new_music, float volume = 1)
        {
            ambience_audio_source.clip = new_music;
            ambience_audio_source.loop = true;
            ambience_audio_source.volume = Mathf.Clamp(volume, 0, music_volume);
            ambience_audio_source.Play();
        }

        public void ChangePlayingAmnienceVolume(float volume)
        {
            ambience_audio_source.volume = Mathf.Clamp(volume, 0, music_volume);
        }
