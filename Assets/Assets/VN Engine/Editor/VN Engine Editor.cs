    ...
    public class VNEngineEditor : MonoBehaviour
    {
        ...
        private static void ImportTxtScriptFile()
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                if (line.StartsWith("ChangeActorEmotion", true, System.Globalization.CultureInfo.InvariantCulture))
                {
                    GameObject go = new GameObject("Enter " + split_line[1]);
                    go.transform.parent = cur_conversation.transform;
                    ChangeActorEmotionNode node = go.AddComponent<ChangeActorEmotionNode>();
                    //node.actor_name = split_line[1];
                }
        ...
        [MenuItem("GameObject/VN Engine/Add Puzzle", false, 0)]
        private static void AddPuzzle(MenuCommand menuCommand)
        {
            GameObject go = new GameObject("Puzzle");     // Create new object
            GameObjectUtility.SetParentAndAlign(go, menuCommand.context as GameObject); // Parent the new object
            Undo.RegisterCreatedObjectUndo(go, "Create " + go.name);    // Register the creation in the undo system
            Selection.activeObject = go;

            go.AddComponent<AddPuzzleNode>();
        }
        ...
        [MenuItem("GameObject/VN Engine/Actors/Change Actor Emotion", false, 5)]
        private static void ChangeActorEmotion(MenuCommand menuCommand)
        {
            GameObject go = new GameObject("Change Actor Emotion");     // Create new object
            GameObjectUtility.SetParentAndAlign(go, menuCommand.context as GameObject); // Parent the new object
            Undo.RegisterCreatedObjectUndo(go, "Create " + go.name);    // Register the creation in the undo system
            Selection.activeObject = go;

            go.AddComponent<ChangeActorEmotionNode>();
        }
